...
for i_episode in range(NUM_EPISODE + 1):      # the first 10000 samples are stored in memory
    ...
    for len in range(LEN_EPISODE):  #length of each episode
        ...
        #print(i_episode, len, dist.detach().numpy(), a)
        ...
        if memory_counter > MEMORY_CAPACITY:
            ...
            b_value = critic(b_s)
            b_next_value = b_r + critic(b_s_)
            critic_loss = actor_criterion(b_value, b_next_value)
            actor_loss = (-b_log * (b_next_value - b_value).detach()).mean() # minus ?
...
plt.plot(result, color = 'red', label = 'AC-batch')
plt.legend()
plt.xlabel('Steps')
plt.ylabel('Average reward over 10 trajectories')
plt.show()
'''
